document.addEventListener('DOMContentLoaded', function () {
    ...
    if (heroCanvas && heroCanvas.getContext) {
        (function () {
            ...
            function pickColor() {
                const r = Math.random();
                if (r < 0.55) return COLORS.white;
                if (r < 0.75) return COLORS.paleBlue;
                if (r < 0.88) return COLORS.blue400;
                if (r < 0.96) return COLORS.pink;
            ...
            function initStars() {
                ...
                stars = new Array(count).fill(0).map(function () {
                    ...
                    const baseR = depth === 0 ? Math.random() * 0.5 + 0.25
                                : depth === 1 ? Math.random() * 0.9 + 0.4
                                              : Math.random() * 1.4 + 0.6;
                    const r = Math.random();
                    const shape = r < 0.78 ? 'dot' : r < 0.94 ? 'sparkle' : 'streak';
                    return {
                        x, y, homeX: x, homeY: y, vx: 0, vy: 0,
                        r: baseR, depth, shape, color: pickColor(),
            ...
            function drawSparkle(s, drawR, alpha, glowBoost) {
                ...
                if (glowBoost > 0.05) {
                    ...
                    ctx.arc(0, 0, arm * 1.4, 0, Math.PI * 2); ctx.fill();
                }
                const gH = ctx.createLinearGradient(-arm, 0, arm, 0);
                ...
                const gV = ctx.createLinearGradient(0, -arm, 0, arm);
                ...
                ctx.arc(0, 0, drawR * 0.9, 0, Math.PI * 2); ctx.fill();
                ctx.restore();
            }

            function drawStreak(s, drawR, alpha) {
                const len = drawR * 5, rgb = s.color.join(',');
                ctx.save(); ctx.translate(s.x, s.y); ctx.rotate(s.angle);
            ...
            function tick() {
                ...
                if (mouse.active) {
                    const hub = ctx.createRadialGradient(mouse.x, mouse.y, 0, mouse.x, mouse.y, R * 0.9);
                    hub.addColorStop(0,   'rgba(91,156,255,0.10)');
                    hub.addColorStop(0.4, 'rgba(43,123,255,0.05)');
                    hub.addColorStop(1,   'rgba(0,27,228,0)');
                    ctx.fillStyle = hub;
                ...
                for (let i = 0; i < stars.length; i++) {
                    ...
                    if (mouse.active) {
                        ...
                        if (d2 < R2) {
                            ...
                            proximity = 1 - d / R;
                            const f = proximity * proximity * MOUSE_FORCE * (0.6 + s.depth * 0.4);
                    ...
                    const alpha = Math.max(0.05, Math.min(s.baseAlpha + tw + proximity * 0.6, 1));
                    const drawR = s.r * (1 + proximity * 1.6);

                    if (s.shape === 'dot') drawDot(s, drawR, alpha);
                    else if (s.shape === 'sparkle') drawSparkle(s, drawR, alpha, proximity);
                    else drawStreak(s, drawR, alpha);
                ...
                if (mouse.active) {
                    ...
                    ctx.arc(mouse.x, mouse.y, 2.2, 0, Math.PI * 2); ctx.fill();
                    ctx.beginPath(); ctx.fillStyle = 'rgba(91,156,255,0.35)';
                    ctx.arc(mouse.x, mouse.y, 6, 0, Math.PI * 2); ctx.fill();
                    ctx.beginPath(); ctx.fillStyle = 'rgba(43,123,255,0.15)';
                    ctx.arc(mouse.x, mouse.y, 14, 0, Math.PI * 2); ctx.fill();
                }
